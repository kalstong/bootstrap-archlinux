...
# NOTE: --needed and --noconfirm need to be placed at the end
# so they're interpreted as pacman's arguments instead of pacstrap's.
pacstrap "$bt_rootdir" mkinitcpio --needed --noconfirm
...
# so they're interpreted as pacman's arguments instead of pacstrap's.
pacstrap "$bt_rootdir" \
	${pacman_core[*]} ${pacman_system[*]} \
	${pacman_tools[*]} ${pacman_development[*]} \
	${pacman_apps[*]} ${pacman_apps[*]} ${pacman_fonts[*]} \
	--needed --noconfirm
